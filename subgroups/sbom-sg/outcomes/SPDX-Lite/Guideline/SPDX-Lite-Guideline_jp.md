...
SPDX Liteファイルを結合したりツールに取り込んだりする場合に一意であることを保証するのは受領側の責任です。
...
本項目は使用しているソフトウェアの実ファイルを特定するために利用します。 これはPackage Nameと実ファイルが名称の違いにより、関連付けできない場合があるため本項目を必要とします。

### L2.5  Package Download Location
ソフトウェア パッケージの入手先（ダウンロードしたURL等）を記載します。
本項目は使用しているソフトウェアと同じソフトウェア（ソースコードやパッケージファイル）を入手するために利用します。物理媒体で納品する場合など、URLが存在しない場合は NOASSERTION と記載します。

### L2.6  Files Analyzed
SPDX Liteファイルを手作業で作成する場合に"false"を記載します。このタグは、自動化ツールなどが、ライセンス情報をソースコードなどのファイルからスキャンし解析したことを示すタグで、手作業でSPDX Liteファイルを作成した場合には、”false”を記載しなければなりません。

### L2.7  Package Home Page
ソフトウェア パッケージのホームページ（プロジェクトや製品のトップページ・紹介サイト等）を記載します。
本項目は使用しているソフトウェアの脆弱性情報などのコミュニティ情報を入手するために利用します。
情報サイト等がない場合は NOASSERTION と記載します。
...
※ソースコードを改変して提供する場合の記載方法については「補足」を参照してください。


#### L2.5 Package Download Location
-> 「https://busybox.net/downloads/busybox-1.30.1.tar.bz2」 あるいは 「https://github.com/提供者/提供パッケージ」 や、「NOASSERTION」

入手したソースコードのダウンロード先、または自社で提供するパッケージの配布先URLを記載します。物理媒体で納品する場合などは NOASSERTION と記載します。

※ソースコードを改変して提供する場合の記載については「補足」を参照してください。
...
SPDX Liteフォーマットによるライセンス情報ファイルは、概ね手作業で作成されることが想定されていますので、多くの場合、このタグはfalseとなります。

#### L2.7 Package Home Page
-> https://busybox.net/about.html

開発プロジェクトが開設しているWebサイト（プロジェクト紹介ページや製品情報ページ）のURLを記載します。個別の受託開発などで情報サイトを設けていない場合などは NOASSERTION と記載します。
...
## 補足: ソースコードに修正を加えて提供する場合
入手したソースコードに修正（パッチ）を加えて配布・納品する場合、提供形態（「元コードとパッチを別々に提供」か「一体で提供」か）によって各タグの記載方法が異なります。
